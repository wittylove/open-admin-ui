  ...
  export default {
    ...
    methods: {
      ...
      getCheckedAuthorities () {
        ...
        const menus1 = this.$refs['tree'].getCheckedProp('parentId')
        let handleLoadauthorityId = this.handleLoadRoleGrantedResdata
        var handleLoadauthorityIddata = new Array()
        for (let i in menus1) {
          for (let j in handleLoadauthorityId) {
            if (menus1[i] == handleLoadauthorityId[j].menuId) {
              handleLoadauthorityIddata[i] = handleLoadauthorityId[j].authorityId
            }
          }
        }
        let menusdistinct = [...new Set(menus.concat(this.formItem.grantActions, handleLoadauthorityIddata))]
        for (let i in menusdistinct) {
          if (menusdistinct[i] === undefined) {
            menusdistinct.splice(i, 1)
            i = i - 1
          }
        }
      ...
      handleLoadUserGranted(userId) {
        ...
        Promise.all([p1, p2]).then(function (values) {
          let res1 = values[0]
          let res2 = values[1]
          that.handleLoadRoleGrantedResdata = res1.data
          if (res1.code === 0 && res1.data) {
            let opt = {
              primaryKey: 'menuId',
              parentKey: 'parentId',
